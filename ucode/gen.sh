...
fetch_intel() {
    ...
    iucode_tool --write-earlyfw="$project/image/boot/intel-ucode.img" intel-ucode{,-with-caveats}/
}
...
fetch_amd() {
    ...
    echo kernel/x86/microcode/AuthenticAMD.bin | bsdtar --uid 0 --gid 0 -cnf - -T - | bsdtar --null -cf - --format=newc @- > "$project/image/boot/amd-ucode.img"
}

mkdir -p "$project/image/boot"
cd "$project/ucode"
[ ! -e "$project/image/boot/intel-ucode.img" ] && fetch_intel
cd "$project/ucode"
[ ! -e "$project/image/boot/amd-ucode.img" ] && fetch_amd